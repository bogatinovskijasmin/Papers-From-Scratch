...
class DiffussionTrainerSupervised(Trainer):
    def __init__(self, max_epochs):
    ...
    def fit_epoch(self):
        ...
        print(f"Training loss {self.training_loss[self.epoch]}")


        sampled_images = self.diffusion.sample(self.model, labels=label, n=images.shape[0])
        self.save_images(sampled_images, os.path.join("results_supervised", f"epoch_{self.epoch}.jpg"))
