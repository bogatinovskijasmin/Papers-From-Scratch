...
class Trainer:
    def __init__(self, max_epochs, scheduler_fcn):
        ...
        self.scheduler_fcn = scheduler_fcn
    ...
    def fit(self, data, model):
        ...
        self.scheduler = self.scheduler_fcn(self.optim, milestones=[3, 6, 9], gamma=0.1)
        self.epoch = 0
    ...
    def fit_epoch(self):
        ...
        # if (self.epoch+1) % 12==0:
            # self.optim.param_groups[0]['lr'] = self.scheduler(self.epoch)
        self.scheduler.step()
        print(f"executing epoch {self.epoch + 1} with learning rate {self.optim.param_groups[0]['lr']}")
