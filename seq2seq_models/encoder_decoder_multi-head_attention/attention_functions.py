...
class MultiHeadAttention(torch.nn.Module):
    def __init__(self, dropout, num_heads, bias=False):
        super().__init__()
        self.num_heads = num_heads
        self.attention = ScaledDotProductAttention(dropout=dropout)
        self.W_q = torch.nn.LazyLinear(num_heads, bias=bias)
        self.W_k = torch.nn.LazyLinear(num_heads, bias=bias)
        self.W_v = torch.nn.LazyLinear(num_heads, bias=bias)
        self.W_o = torch.nn.LazyLinear(num_heads, bias=bias)
    ...
    def forward(self, queries, keys, values, valid_seq_lens):
        ...
        if valid_seq_lens is not None:
            valid_seq_lens = torch.repeat_interleave(valid_seq_lens, self.num_heads)
