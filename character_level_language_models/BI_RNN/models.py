...
class Bidirectional_GRU(torch.nn.Module):
    def __init__(self, num_inputs, num_hiddens, sigma, num_blocks):
        super().__init__()
        self.rnn_fwd = StackedRNN(num_inputs=num_inputs, num_hiddens=num_hiddens, sigma=sigma, num_blocks=num_blocks)
        self.rnn_bkw = StackedRNN(num_inputs=num_inputs, num_hiddens=num_hiddens, sigma=sigma, num_blocks=num_blocks)
    ...
    def forward(self, X, states=None):
        ...
        X_fwd, states_fwd = self.rnn_fwd.forward(X, states_fwd)
        X_bkw, states_bkw = self.rnn_bkw.forward(reversed(X), states_bkw)
        X = torch.cat([X_fwd, reversed(X_bkw)], dim=-1)
        return X, states
...
class LanguageModel(Classifier):
    ...
    def output_layer(self):
        self.W_o = torch.nn.Parameter(torch.normal(0, self.sigma, (2*self.num_hiddens, self.num_outputs)))
